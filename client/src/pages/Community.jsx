...
export default function CommunityFeedback() {
  ...
  const loadFeedback = async () => {
    try {
      ...
      const userRatings = {};
      res.data.forEach((fb) => {
        const myRating = fb.ratings?.find(r => r.userId === (user?.id || user?._id));
  ...
  useEffect(() => {
    ...
    socketRef.current.on("feedback:reply", ({ feedbackId, reply }) => {
      setFeedbackList(prev => prev.map(f => f._id === feedbackId ? { ...f, replies: [...(f.replies||[]), reply] } : f));
    });

    socketRef.current.on("feedback:rating", ({ feedbackId, avgRating }) => {
      setFeedbackList(prev => prev.map(f => f._id === feedbackId ? { ...f, avgRating } : f));
    });
  ...
  const Star = ({ filled }) => (
    <svg className={`w-5 h-5 inline-block ${filled ? "text-yellow-400" : "text-gray-300"}`} viewBox="0 0 20 20" fill="currentColor">
  ...
  if (!token) {
    return (
      <div className="p-4 sm:p-6 max-w-full sm:max-w-3xl mx-auto text-center ml-24">
        <div className="fixed left-0 top-0 h-full">
          <Navbar />
        </div>
        <h2 className="text-xl font-semibold">Please login to access the community feedback</h2>
      </div>
    );
  }

  return (
    <div className="p-4 sm:p-6 max-w-full sm:max-w-4xl mx-auto ml-24">
        ...
        <div className="flex flex-col sm:flex-row items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mt-2 gap-2">
              ...
              <button onClick={submitFeedback} className="bg-green text-white px-4 py-2 rounded-lg">
                Submit
              </button>
            </div>
          </div>
      ...
      <div className="space-y-4">
        {feedbackList.map((fb) => (
          <div key={fb._id} className="bg-white shadow p-4 rounded-xl">
            <div className="flex flex-col sm:flex-row items-start gap-3">
              <div className="w-10 h-10 rounded-full bg-green text-white flex items-center justify-center font-bold">
              ...
              <div className="flex-1">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between">
                  <div>
                    <div className="font-semibold text-gray-800">{fb.user?.username}</div>
                  ...
                  <div className="text-sm text-gray-600 mt-1 sm:mt-0">
                    Avg: <strong>{fb.avgRating ? fb.avgRating : "—"}</strong>
                  </div>
                </div>
                ...
                <div className="mt-3 flex items-center gap-1">
                  {[1,2,3,4,5].map((n) => {
                    ...
                    return (
                      <button key={n} onClick={() => submitRating(fb._id, n)} className="focus:outline-none">
                        <Star filled={filled} />
                      </button>
                    );
                  })}
                  <div className="text-xs text-gray-500 ml-2">({fb.ratings?.length || 0} votes)</div>
                </div>

                <div className="mt-4 space-y-2">
                  {(fb.replies || []).map((r) => (
                    <div key={r._id || r.createdAt} className="bg-gray-50 p-3 rounded">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between">
                ...
                <div className="mt-3 flex flex-col sm:flex-row gap-2">
                  <input
                    ...
                    onChange={(e) => setReplyText(prev => ({ ...prev, [fb._id]: e.target.value }))}
                  />
                  <button onClick={() => submitReply(fb._id)} className="bg-blue text-white px-3 py-1 rounded">
                    Reply
                  </button>
                </div>
